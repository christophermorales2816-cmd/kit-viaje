...
-- ---------------------------------------------------------------------------
-- 11. create_trip es atómica y solo la puede llamar service_role
--
-- La razón de que la creación sea una función y no tres inserts: PostgREST
-- corre cada llamada en su propia transacción, así que un fallo a mitad de
-- camino dejaría un viaje sin equipaje ni presupuesto, y el usuario aterrizaría
-- en un dashboard vacío sin ninguna pista.
-- ---------------------------------------------------------------------------
...
begin
  begin
    perform public.create_trip(
      '11111111-1111-1111-1111-111111111111', '2026-07-10', '2026-07-14', 'urbano',
      '[{"item_id":"22222222-2222-2222-2222-222222222222","qty":1},
        {"item_id":"00000000-0000-4000-8000-999999999999","qty":1}]'::jsonb,
      '[]'::jsonb);
    raise exception 'FALLO: create_trip aceptó un item_id inexistente';
  exception
    when foreign_key_violation then
      select count(*) into n from trips where start_date = '2026-07-10';
      if n <> 0 then
        raise exception 'FALLO: quedó un viaje huérfano tras el error';
      end if;
      raise notice 'OK  create_trip aborta entera si un ítem es inválido';
  end;
end $$;
...
begin
  perform public.create_trip(
    '11111111-1111-1111-1111-111111111111', '2026-07-10', '2026-09-30', 'urbano',
    '[]'::jsonb, '[]'::jsonb);
  raise exception 'FALLO: create_trip aceptó un viaje de más de 30 días';
exception
  when check_violation then
    raise notice 'OK  los constraints del viaje aplican dentro de create_trip';
end $$;
...
begin
  ...
  perform public.create_trip(
    '11111111-1111-1111-1111-111111111111', '2026-07-10', '2026-07-14', 'urbano',
    '[]'::jsonb, '[]'::jsonb);
  raise exception 'FALLO: anon pudo ejecutar create_trip';
exception
  when insufficient_privilege then
    raise notice 'OK  anon no puede ejecutar create_trip';
end $$;
reset role;
...
begin
  set local role service_role;
  select * into creado from public.create_trip(
    '11111111-1111-1111-1111-111111111111', '2026-07-10', '2026-07-14', 'urbano',
    '[{"item_id":"22222222-2222-2222-2222-222222222222","qty":5}]'::jsonb,
    '[{"product_id":"33333333-3333-3333-3333-333333333333","qty":5}]'::jsonb);

  if creado.edit_token is null or creado.share_slug is null then
    raise exception 'FALLO: create_trip no devolvió los tokens';
  end if;
  raise notice 'OK  service_role crea el viaje con sus dos listas';
end $$;
reset role;
